...
def get_user_input():
    ...
    n_bandits = int(input("How many bandits? "))
    ...
    for b in range(n_bandits):
        print()
        b_name = input(f"  Name of bandit {b+1}: ").strip()
        bandit_names.append(b_name)

        n_arms = int(input(f"  How many arms for '{b_name}'? "))
        arm_counts.append(n_arms)

        while True:
            btype = input(f"  Type for '{b_name}' (early / late / ongoing): ").strip().lower()
            if btype in ['early', 'late', 'ongoing']:
                bandit_types.append(btype)
                break
            print("    Invalid type. Please enter 'early', 'late', or 'ongoing'.")

        bandits_template[b_name] = {}
        for a in range(n_arms):
            a_name = input(f"    Name of arm {a+1} for '{b_name}': ").strip()
            bandits_template[b_name][a] = [[1, 1], a_name]

    return bandits_template, bandit_names, arm_counts, bandit_types
...
bandits_template, bandit_names, arm_counts, bandit_types = get_user_input()
n_bandits = len(bandit_names)

tests = generate_tests(arm_counts, n_tests=9)
...
for noise in noise_levels:
    for _ in range(runs_per_test):
        for test in tests:
            ...
            bandit = five_bandit(bandits, bandit_types, initial_bias, optimal_arm, total_rounds, noise)
...
with open('dynamic_global_bandit_results.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['rowid', 'runID', 'noise_level', 'round', 'performance'])
    writer.writerows(rows)

print(f"\nSaved {len(rows)} rows to dynamic_global_bandit_results.csv")