...
class dynamic_bandit():
    ...
    def main(self):
        ...
        for i in range(total_rounds):
            ...
            # Posterior normalization using bandit types
            all_posterior = []
            ...
            if i == 0:
                ...
                for idx, b_name in enumerate(bandit_names):
                    ...
                    arms_chosen[idx] = np.random.choice(range(n_arms), p=all_posterior[idx])
...
filename  = f"non_global_results_rounds{total_rounds}_dims{n_bandits}_tests{n_tests}_{timestamp}.csv"
save_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Non Global results")
