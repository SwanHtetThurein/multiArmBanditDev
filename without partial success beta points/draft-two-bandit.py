...
class two_bandit():
    ...
    def reward_generator(self, curr_arms, optimal_arms, noise=0.3):
        """
        ...
        for chosen, opt in zip(curr_arms, optimal_arms):
            if chosen != opt:
                ...
                p += noise * 0.4    # small bonus for correct arm
    ...
    def main(self):
        ...
        return 1 if (Hierarchy_max_arm_index == self.optimal_arm[0] and Interaction_max_arm_index == self.optimal_arm[1]) else 0.5 if (Hierarchy_max_arm_index == self.optimal_arm[0] or Interaction_max_arm_index == self.optimal_arm[1]) else 0
