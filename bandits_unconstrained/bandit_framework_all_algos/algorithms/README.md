...
This document covers the 21 algorithms registered in `__init__.py`. The parent
...
**(a) Acquisition rule, surrogate held fixed.** `bocs`, `linucb`, `kg` and
`purexp` share the identical second-order Bayesian linear model, identical
priors, and the identical local-search optimizer. They differ *only* in how
they choose the next team:
...
Any difference among these four is attributable to the acquisition rule and
nothing else. `purexp` against `random` isolates the value of the *model*;
`purexp` against the other three isolates the value of *adaptive allocation*.
...
| moderate | `bayesgap`, `gp_onehot`, `neurallinear` | 3 – 6 min |
| heavy | `kg`, `bocs_hs`, `bootnn`, `combo`, `combo_slice`, `linucb` | 8 – 18 min |
| heaviest | `bocs`, `smac` | ~34 min each |

Running all 21 is on the order of three hours.
...
| BOCS feature map | intercept + first-order + pairwise, ~470 features | `bocs`, `bocs_hs`, `linucb`, `kg`, `purexp`, `bayesgap` |
| Prior scales | `G_FIRST = 1.0`, `G_SECOND = 0.25` | the same six |
| Noise prior | `InvGamma(A0=2.0, B0=0.05)` | the same six |
